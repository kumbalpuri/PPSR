...
interface SidebarProps {
  ...
  activePpsrTab?: string;
  setActivePpsrTab?: (tab: any) => void;
  persona: UserPersona;
...
export default function Sidebar({
  ...
  const selectSubTab = (moduleKey: any, subTab: string, actionVal?: string) => {
    ...
    if (moduleKey === 'kaizen') {
      ...
      if (setActivePpsrTab) {
        setActivePpsrTab(subTab as any);
      }
      setInitialPpsrAction(actionVal || subTab || null);
    }
    setIsMobileOpen(false);
  };
                ...
                <button
                  ...
                  className={`w-full flex items-center space-x-2 px-2.5 py-1.5 rounded-lg text-left text-[11px] font-semibold transition ${
                    activeModule === 'ppsr' && activePpsrTab === 'meeting'
                      ? 'text-emerald-300 font-bold bg-slate-900/80 shadow-xs'
                      : 'text-slate-400 hover:text-emerald-300 hover:bg-slate-900/40'
                  }`}
                >
                ...
                <button
                  ...
                  className={`w-full flex items-center space-x-2 px-2.5 py-1.5 rounded-lg text-left text-[11px] font-semibold transition ${
                    activeModule === 'ppsr' && activePpsrTab === 'cft-awards'
                      ? 'text-amber-300 font-bold bg-slate-900/80 shadow-xs'
                      : 'text-amber-400/90 hover:text-amber-300 hover:bg-slate-900/40'
                  }`}
                >
                  <Trophy className="w-3 h-3 shrink-0 text-amber-400" />
